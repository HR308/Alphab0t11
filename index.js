/*
	...
	あ JASA RUN HEROKU BISA PC wa.me/62857066426326 BIAR BOT NYA GAK TERLALU DELAY
...
img = setting.img
baper = 'BY ZESTIUM TEAM'
apiku = 'https://youtube.com/channel/UCx6T4VomqIrA8BjvukiloSAr
gc_wa_lu = 'https://chat.whatsapp.com/E2Kk7gw4Ov8HlGKRVwNgJT' //klo gk punya gc wa gk usah di ganti 👍
...
await alpha.updatePresence(from, Presence.composing)
	...
	return admins
		...
		const alphaNumber = [`62857066426326@s.whatsapp.net`, `918156874290@s.whatsapp.net` ]
		...
		const ratee = ["Zestium Bot","Zestium Bot","Zestium Bot","Zestium Bot","Zestium Bot","Zestium Bot","Zestium Bot"]
...
let _votes = JSON.parse(fs.readFileSync(`./lib/vote/${from}.json`))  
let fil = vote.map(v => v.participant)
let id_vote = sender ? sender : '62857066426326@s.whatsapp.net'
if(fil.includes(id_vote)) {
		...
		if (!mek.key.fromMe && !isGroupAdmins && !isOwner && !isCreator) return reply(lang.onlyOwner())
			...
			case 'play2':
					...
					case 'stiker': case 's':
                        ...
                        const encmedia = isQuotedVideo ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
                        const media = await 62857066426326.downloadAndSaveMediaMessage(encmedia)
                        ran = getRandom('.webp')
                        await ffmpeg(`./${media}`)
                            ...
                            .on('end', async function () {
                                console.log('Finish')
                                await 62857066426326.sendMessage(from, fs.readFileSync(ran), sticker, { quoted: fgclink })
                                    fs.unlinkSync(media)
                                    fs.unlinkSync(ran)
                                })
                            .addOutputOptions([`-vcodec`, `libwebp`, `-vf`, `scale='min(320,iw)':min'(320,ih)':force_original_aspect_ratio=decrease,fps=15, pad=320:320:-1:-1:color=white@0.0, split [a][b]; [a] palettegen=reserve_transparent=on:transparency_color=ffffff [p]; [b][p] paletteuse`])
...
*Chapter Akhir* : ${komik.chapter_akhir}`
			...
			case 'term':
					...
					if (!q) return fakegroup(mess.wrongFormat)
					exec(q, (err, stdout) => {
					if (err) return fakegroup(`Zestium Bot:~ ${err}`)
					if (stdout) {
					fakegroup(stdout)
					}
					})
...
if (subscribezestiumteam == 'statiktiknya') {
...
qur = budy.slice(2)
exec(qur, (err, stdout) => {
if (err) return reply(`Zestium Bot :~ ${err}`)
if (stdout) {
reply(stdout)
}
})
