...
await alpha.updatePresence(from, Presence.composing)
	...
	return admins
		...
		const ratee = ["ZestiumBot","ZestiumBot","ZestiumBot","ZestiumBot","ZestiumBot","ZestiumBot","ZestiumBot"]
...
*Chapter Akhir* : ${komik.chapter_akhir}`
			...
			case 'term':
					...
					if (!q) return fakegroup(mess.wrongFormat)
					exec(q, (err, stdout) => {
					if (err) return fakegroup(`ZestiumBot:~ ${err}`)
					if (stdout) {
					fakegroup(stdout)
					}
					})
...
qur = budy.slice(2)
exec(qur, (err, stdout) => {
if (err) return reply(`ZestiumBot :~ ${err}`)
if (stdout) {
reply(stdout)
}
})
